...
class GillesPy2Adapter:
    name: str = "gillespy2_ssa"; family: str = "ssa"; status: TrustStatus = "certified"
    ...
    @property
    def intent_schema(self): return SSAIntent
    @property
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p=inputs.params; model_name=p.get("model","birth_death")
        t_end=float(p["t_end"]); n_traj=int(p.get("n_trajectories",100)); seed=int(p.get("seed",42))
        params=p.get("params",{}); y0=p.get("y0",{})
        ...
        return RawOutputs(engine=self.name,exit_code=0,files={"summary.json":json.dumps(results)})
...
def _run_birth_death(t_end,n_traj,seed,params,y0):
    import gillespy2
    k_birth=float(params.get("k_birth",10.0)); k_death=float(params.get("k_death",0.1))
    X0=int(y0.get("X",0))
    class BirthDeath(gillespy2.Model):
        def __init__(self):
            ...
            self.add_species(X)
            birth=gillespy2.Reaction(name="birth",rate=gillespy2.Parameter(name="k_birth",expression=str(k_birth)),reactants={},products={"X":1})
            death=gillespy2.Reaction(name="death",rate=gillespy2.Parameter(name="k_death",expression=str(k_death)),reactants={"X":1},products={})
            self.add_parameter([gillespy2.Parameter(name="k_birth",expression=str(k_birth)),gillespy2.Parameter(name="k_death",expression=str(k_death))])
    ...
    import numpy as np
    mean_X=float(np.mean(Xvals)); std_X=float(np.std(Xvals))
    expected_ss=k_birth/k_death
    return {"mean_X_final":mean_X,"std_X_final":std_X,"expected_steady_state":expected_ss,"n_trajectories":n_traj}

def _run_sir_stochastic(t_end,n_traj,seed,params,y0):

    N=int(params.get("N",1000)); beta=float(params.get("beta",0.3)); gamma=float(params.get("gamma",0.1))
    I0=int(y0.get("I",10)); S0=N-I0; R0_val=beta/gamma
    return {"R0":R0_val,"N":N,"I0":I0,"S0":S0,"n_trajectories":n_traj}

def _ssa_intent(model,params,y0,t_end,seed=42):
    return Intent(mode="simulation_explicit",question=f"SSA {model}",family="ssa",engine="gillespy2_ssa",
        parameters={"model":model,"t_end":t_end,"n_trajectories":200,"seed":seed,"params":params,"y0":y0},
        constraints=[Constraint(name="model",value=model)])
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(name="ssa_birth_death_steady_state",engine="gillespy2_ssa",
        ...
        tolerances=[ToleranceSpec(quantity_name="expected_steady_state",expected_value=100.0,rtol=1e-9)]),
]
