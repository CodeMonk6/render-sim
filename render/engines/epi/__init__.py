...
class EpiAdapter:
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        I_t = sol.y[idx]
        R_final = float(sol.y[-1, -1])
        peak_I = float(I_t.max())
        peak_t = float(sol.t[int(I_t.argmax())])
        ...
        series = {
            ...
            "y": [
                {"name": lab, "values": [round(float(v), 3) for v in sol.y[i][::step]]}
        ...
        s = {
            ...
            "peak_t_days": peak_t,
            "R_final": R_final,
            "attack_rate": R_final / N,
            "S_final": float(sol.y[0, -1]),
            "series": series,
        }
...
def _intent(model, N, beta, gamma, I0, t_end):
    return Intent(
        ...
        question=f"{model} R0={beta / gamma:.1f}",
        family="epi",
        engine="epi_sir_seir",
        parameters={
            "model": model,
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(
        name="sir_r0_threshold",
        engine="epi_sir_seir",
        description="SIR: R0 = beta/gamma exactly",
        citation="Kermack & McKendrick (1927)",
        intent=_intent("SIR", 10000.0, 0.3, 0.1, 1.0, 160.0),
        tolerances=[ToleranceSpec(quantity_name="R0", expected_value=3.0, rtol=1e-9)],
    ),
    ReferenceCase(
        name="sir_subcritical",
        engine="epi_sir_seir",
        description="SIR R0<1: attack_rate near 0",
        citation="Anderson & May (1991)",
        intent=_intent("SIR", 10000.0, 0.05, 0.1, 1.0, 200.0),
        tolerances=[
            ToleranceSpec(quantity_name="R0", expected_value=0.5, rtol=1e-9),
