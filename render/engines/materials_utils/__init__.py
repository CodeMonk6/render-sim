...
class MaterialsIntent(BaseModel):
    ...
    formula: str = Field(default="", description="Chemical formula or material name, e.g. 'Si', 'NaCl', 'H2O'")
    crystal_structure: str = Field(default="", description="Spacegroup or prototype, e.g. 'fcc', 'rocksalt', 'diamond'")
    lattice_constant_ang: float = Field(default=0.0, description="Lattice constant in Å (0 = use default)")
    input_file: str = Field(default="", description="Input structure text (CIF, POSCAR, XYZ) for convert/analyze")
    input_format: str = Field(default="", description="Input file format: 'cif','poscar','xyz','pdb'")
...
class ASEAdapter:
    ...
    regime: RegimeSpec = RegimeSpec(bounds=[],notes="Utility operations; no physical regime bounds.")
    @property
    def intent_schema(self): return MaterialsIntent
    @property
    ...
    def validate(self, intent: Intent) -> ValidationReport:
        ...
        valid_ops={"build_bulk","build_molecule","convert_structure","analyze_structure","supercell","surface_slab"}
        if op not in valid_ops:
            return ValidationReport(passed=False,failed_layer=1,errors=[f"Unknown operation '{op}'. Valid: {sorted(valid_ops)}"])
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p=inputs.params; op=p.get("operation","build_bulk")
        result: dict = {}
        atoms = None
        if op=="build_bulk":
            ...
            try: atoms=molecule(formula)
            except KeyError: return RawOutputs(engine=self.name,exit_code=1,stderr=f"Unknown molecule '{formula}'",files={})
            result={"formula":formula,"n_atoms":len(atoms),"positions":atoms.get_positions().tolist()}
        elif op in ("convert_structure","analyze_structure"):
            raw_text=p.get("input_file",""); fmt=p.get("input_format","xyz")
            if not raw_text:
                return RawOutputs(engine=self.name,exit_code=1,stderr="input_file required",files={})
            f=io.StringIO(raw_text); atoms=ase_read(f,format=fmt)
            result={"n_atoms":len(atoms),"chemical_formula":str(atoms.get_chemical_formula()),
            ...
            result={"n_atoms":len(atoms),"supercell_matrix":sc,"cell_volume_ang3":float(atoms.get_volume())}
        elif op=="surface_slab":
            formula=p.get("formula","Si"); cs=p.get("crystal_structure","diamond")
            mi=p.get("miller_index",[0,0,1]); layers=int(p.get("slab_layers",4))
            from ase.build import surface
            slab=surface(bulk(formula,crystalstructure=cs),mi,layers)
            result={"n_atoms":len(slab),"miller_index":mi,"n_layers":layers,"cell_volume_ang3":float(slab.get_volume())}
        else:
            result={}
        # Write output structure as XYZ if atoms available
        out_files: dict[str,str]={"summary.json":json.dumps(result)}
        if atoms is not None:
            buf=io.StringIO(); ase_write(buf,atoms,format=p.get("output_format","xyz")); out_files["structure.xyz"]=buf.getvalue()
    ...
    def parse(self, raw: RawOutputs) -> ResultBundle:
        ...
        if "cell_volume_ang3" in s: quantities.append(Quantity(name="cell_volume_ang3",value=s["cell_volume_ang3"],unit="Å³"))
        return ResultBundle(engine=self.name,quantities=quantities,converged=True,metadata=s)

def _mat_intent(op,formula,cs,title=""):
    return Intent(mode="simulation_explicit",question=title or f"{op} {formula}",family="materials_utils",
        engine="ase_materials",parameters={"operation":op,"formula":formula,"crystal_structure":cs},
        constraints=[Constraint(name="operation",value=op)])
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(name="ase_si_bulk_volume",engine="ase_materials",
    ...
    ReferenceCase(name="ase_h2o_molecule_atoms",engine="ase_materials",
