...
class GROMACSAdapter:
    name: str = "gromacs_md"; family: str = "md"; status: TrustStatus = "experimental"
    version: ClassVar[str] = "1.0.0"; runtime: ClassVar[str] = "either"
    environment: EnvSpec = EnvSpec(env_type="conda",packages=["gromacs>=2023.4"],module_name="GROMACS",
    ...
    @property
    def intent_schema(self): return GROMACSIntent
    @property
    ...
    def validate(self, intent: Intent) -> ValidationReport:
        ...
        if intent.parameters.get("system","") not in valid:
            return ValidationReport(passed=False,failed_layer=1,errors=["Unknown GROMACS system."])
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        if subprocess.run(["which",gmx],capture_output=True).returncode!=0:
            gmx="gmx_mpi"
            if subprocess.run(["which",gmx],capture_output=True).returncode!=0:
                raise ImportError("GROMACS not found. module load GROMACS on Compute2, or set GMX_CMD.")
        # Stub: full run requires topology/coordinate input files (certify on Compute2)
        T=float(p.get("temperature",300.0)); P=float(p.get("pressure_bar",1.0))
        s={"system":p.get("system","ala_dipeptide_em"),"temperature_K":T,"pressure_bar":P,
           "converged":False,"note":"Run on Compute2 with topology files to certify."}
...
def _i(sys_,T,nsteps): return Intent(mode="simulation_explicit",question=f"GROMACS {sys_}",family="md",engine="gromacs_md",
    parameters={"system":sys_,"temperature":T,"n_steps":nsteps,"seed":42},constraints=[Constraint(name="system",value=sys_)])
_REF: list[ReferenceCase]=[
    ReferenceCase(name="gromacs_ala_em",engine="gromacs_md",description="Alanine dipeptide EM — adapter dispatches without error",
        citation="GROMACS tutorial",intent=_i("ala_dipeptide_em",300.0,1000),
