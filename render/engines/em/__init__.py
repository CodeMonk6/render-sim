...
class MeepAdapter:
    ...
    environment: EnvSpec = EnvSpec(
        env_type="conda", packages=["meep>=1.29"], notes="conda install -c conda-forge meep"
    )
    regime: RegimeSpec = RegimeSpec(
        bounds=[RegimeBound(field="resolution", min_val=4, max_val=128)],
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p = inputs.params
        prob = p.get("problem", "waveguide")
        wl = float(p.get("wavelength_um", 1.55))
        res = int(p.get("resolution", 16))
        rt = float(p.get("run_time", 200.0))
        n = float(p.get("n_medium", 3.4))
        freq = 1.0 / wl
        if prob == "waveguide":
            ...
            geometry = [
                mp.Block(
                    mp.Vector3(mp.inf, 1, mp.inf), center=mp.Vector3(), material=mp.Medium(index=n)
                )
            ]
            sources = [
                mp.Source(
                    mp.ContinuousSource(frequency=freq), component=mp.Ez, center=mp.Vector3(-5, 0)
                )
            ]
            sim = mp.Simulation(
                cell_size=cell,
            ...
            s = {
                ...
                "ez_max": 0.0,
                "converged": False,
                "note": "Not implemented in local mode",
            }
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(
        name="meep_waveguide_ez_nonzero",
        engine="meep_em",
