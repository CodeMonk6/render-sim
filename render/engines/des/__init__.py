...
class SimPyAdapter:
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p = inputs.params
        p.get("model", "mm1_queue")
        sim_time = float(p["sim_time"])
        lam = float(p.get("arrival_rate", 1.0))
        mu = float(p.get("service_rate", 2.0))
        n_srv = int(p.get("n_servers", 1))
        ...
        res = simpy.Resource(env, capacity=n_srv)
...
def _des_intent(model, sim_time, lam, mu, n_srv, seed=42):
    return Intent(
        ...
        question=f"{model} lambda={lam} mu={mu}",
        family="des",
        engine="simpy_des",
        parameters={
            "model": model,
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(
        name="des_mm1_utilization",
        engine="simpy_des",
        description="M/M/1 queue: rho = lambda/mu exactly",
        citation="Kleinrock (1975) Vol 1",
        intent=_des_intent("mm1_queue", 10000.0, 1.0, 2.0, 1, 42),
        tolerances=[
            ToleranceSpec(quantity_name="utilization_theory", expected_value=0.5, rtol=1e-9)
        ],
    ),
]
