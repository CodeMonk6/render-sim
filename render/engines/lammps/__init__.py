...
class LAMMPSAdapter:
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        try:
            ...
            T = float(p.get("temperature", 100.0))
            nsteps = int(p.get("n_steps", 10000))
            eps = float(p.get("lj_epsilon_kcal", 0.2385))
            sig = float(p.get("lj_sigma_ang", 3.405))
            a = float(p.get("density_fcc_a", 5.26))
            ts = float(p.get("timestep_fs", 2.0))
            seed = int(p.get("seed", 12345))
            script = _LJ_SCRIPT.format(
                a=a, epsilon=eps, sigma=sig, temperature=T, seed=seed, timestep=ts, n_steps=nsteps
            )
            ...
            for line in script.splitlines():
                if line.strip():
                    L.command(line)
            s = {
                "system": p.get("system", "lj_argon"),
                "temperature_K": float(L.get_thermo("temp")),
                "density_g_cm3": float(L.get_thermo("density")),
                "pe_kcal_mol": float(L.get_thermo("pe")),
                "converged": True,
            }
            L.close()
        ...
        if subprocess.run(["which", lmp], capture_output=True).returncode != 0:
            raise ImportError(
                "LAMMPS not found. module load LAMMPS on Compute2, or set LAMMPS_CMD."
            )
        T = float(p.get("temperature", 100.0))
        nsteps = int(p.get("n_steps", 10000))
        eps = float(p.get("lj_epsilon_kcal", 0.2385))
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            inp = Path(tmpdir) / "in.lammps"
            inp.write_text(script)
            proc = subprocess.run(
                [lmp, "-in", str(inp)], capture_output=True, text=True, cwd=tmpdir, timeout=3600
            )
            ...
            if log.exists():
                for line in log.read_text().splitlines():
                    ...
                    if len(parts) >= 5 and parts[0].isdigit():
                        try:
                            temp_o = float(parts[1])
                            pe_o = float(parts[3])
                            dens_o = float(parts[4])
                        except (IndexError, ValueError):
                            pass
            s = {
                "system": p.get("system", "lj_argon"),
                "temperature_K": temp_o,
                "density_g_cm3": dens_o,
                "pe_kcal_mol": pe_o,
                "converged": True,
            }
            return RawOutputs(engine=self.name, exit_code=0, files={"summary.json": json.dumps(s)})
...
def _i(T, nsteps):
    return Intent(
        ...
        question=f"LAMMPS LJ argon T={T}K",
        family="md",
        engine="lammps_md",
        parameters={"system": "lj_argon", "temperature": T, "n_steps": nsteps, "seed": 12345},
        constraints=[Constraint(name="system", value="lj_argon")],
    )


_REF: list[ReferenceCase] = [
    ReferenceCase(
        name="lammps_lj_argon_density",
        engine="lammps_md",
        description="LJ argon NVT 85K — density ~1.4 g/cm³",
        citation="Rahman (1964)",
        intent=_i(85.0, 5000),
        tolerances=[ToleranceSpec(quantity_name="density_g_cm3", expected_value=1.4, rtol=0.15)],
    ),
    ReferenceCase(
        name="lammps_lj_argon_temp",
        engine="lammps_md",
        description="NVT thermostat holds temperature ±20%",
        citation="Standard NVT",
        intent=_i(100.0, 2000),
        tolerances=[ToleranceSpec(quantity_name="temperature_K", expected_value=100.0, rtol=0.2)],
    ),
]
