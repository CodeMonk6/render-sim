...
class ReboundAdapter:
    name: str = "rebound_nbody"; family: str = "nbody"; status: TrustStatus = "certified"
    version: ClassVar[str] = "1.0.0"; runtime: ClassVar[str] = "local"
    environment: EnvSpec = EnvSpec(env_type="pip", packages=["rebound>=3.28"])
    regime: RegimeSpec = RegimeSpec(bounds=[RegimeBound(field="t_end",min_val=1e-3,max_val=1e6,unit="years")])
    @property
    def intent_schema(self): return NBodyIntent
    @property
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p=inputs.params; model=p.get("model","kepler_orbit")
        t_end=float(p["t_end"]); integrator=p.get("integrator","ias15")
        params=p.get("params",{})
...
def _kepler(t_end,integrator,params):
    import rebound
    a=float(params.get("a",1.0)); e=float(params.get("e",0.0)); m_star=float(params.get("m_star",1.0))
    sim=rebound.Simulation(); sim.integrator=integrator; sim.units=("yr","AU","Msun")
    sim.add(m=m_star); sim.add(a=a,e=e)
    T_kepler=2*math.pi*a**1.5/m_star**0.5
    sim.integrate(t_end)
    p=sim.particles[1]; r=math.sqrt(p.x**2+p.y**2+p.z**2)
    return {"period_kepler_years":T_kepler,"semi_major_axis":a,"eccentricity":e,
            "r_final_AU":r,"t_end_years":t_end}

def _nbody_intent(model,t_end,params):
    return Intent(mode="simulation_explicit",question=f"N-body {model}",family="nbody",engine="rebound_nbody",
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(name="nbody_kepler_period",engine="rebound_nbody",
        ...
        tolerances=[ToleranceSpec(quantity_name="period_kepler_years",expected_value=2*math.pi,rtol=1e-9)]),
]
