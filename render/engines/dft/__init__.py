...
class PySCFAdapter:
    ...
    @property
    def intent_schema(self): return DFTIntent
    @property
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        mol.charge=charge; mol.spin=spin; mol.verbose=0; mol.build()
        if method=="hf":
            mf=scf.RHF(mol) if spin==0 else scf.UHF(mol)
        elif method=="dft":
            xc=p.get("xc",p.get("functional","b3lyp"))
            mf=pyscf_dft.RKS(mol) if spin==0 else pyscf_dft.UKS(mol); mf.xc=xc
        else:
            mf=scf.RHF(mol)
        e=mf.kernel(); converged=bool(mf.converged)
        s={"energy_hartree":float(e),"energy_eV":float(e)*27.2114,"converged":converged,
           "molecule":mol_str,"method":method,"basis":basis,"n_electrons":int(mol.nelectron)}
        return RawOutputs(engine=self.name,exit_code=0 if converged else 1,files={"summary.json":json.dumps(s)})
    def parse(self, raw: RawOutputs) -> ResultBundle:
        ...
        ],converged=s["converged"],metadata=s)

def _dft_intent(mol,method,basis,t="DFT calc"):
    return Intent(mode="simulation_explicit",question=t,family="dft",engine="pyscf_dft",
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(name="dft_h2_hf_sto3g_energy",engine="pyscf_dft",
        ...
        tolerances=[ToleranceSpec(quantity_name="energy_hartree",expected_value=-1.1175,rtol=0.001)]),
    ReferenceCase(name="dft_h2o_hf_sto3g_energy",engine="pyscf_dft",
        ...
        tolerances=[ToleranceSpec(quantity_name="energy_hartree",expected_value=-74.96,rtol=0.01)]),
]
