...
class SBMLIntent(BaseModel):
    model_sbml: str = Field(
        description="SBML/Antimony model string or built-in name ('goodwin', 'repressilator')"
    )
...
class TelluriumAdapter:
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p = inputs.params
        model_str = p.get("model_sbml", "repressilator")
...
def _sbml_intent(model_str, t_end):
    return Intent(
        ...
        question=f"SBML {model_str} t={t_end}",
        family="sbml",
        engine="tellurium_sbml",
        parameters={"model_sbml": model_str, "t_end": t_end, "n_points": 500},
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(
        name="sbml_repressilator_species_positive",
        engine="tellurium_sbml",
