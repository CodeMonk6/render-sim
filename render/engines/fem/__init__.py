...
class FEniCSxAdapter:
    ...
    environment: EnvSpec = EnvSpec(
        env_type="conda",
        packages=["fenics-dolfinx>=0.8"],
        notes="conda install -c conda-forge fenics-dolfinx",
    )
    regime: RegimeSpec = RegimeSpec(
        bounds=[RegimeBound(field="n_cells", min_val=2, max_val=512)],
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        try:
            ...
            raise ImportError(
                "FEniCSx not installed. Run: conda install -c conda-forge fenics-dolfinx"
            )
        p = inputs.params
        prob = p.get("problem", "poisson")
        n = int(p.get("n_cells", 32))
        domain = dmesh.create_unit_square(MPI.COMM_WORLD, n, n)
        V = fem.functionspace(domain, ("Lagrange", int(p.get("degree", 1))))
        u, v = ufl.TrialFunction(V), ufl.TestFunction(V)
        f_val = float(p.get("f_source", "1.0"))
        f = fem.Constant(domain, f_val)
        a = ufl.dot(ufl.grad(u), ufl.grad(v)) * ufl.dx
        L = f * v * ufl.dx
        bc = fem.dirichletbc(
            fem.Constant(domain, float(p.get("dirichlet_value", 0.0))),
            fem.locate_dofs_topological(
                V,
                domain.topology.dim - 1,
                dmesh.locate_entities_boundary(
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(
        name="fem_poisson_u_max_positive",
        engine="fenicsx_fem",
