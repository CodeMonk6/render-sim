...
class MesaAdapter:
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p = inputs.params
        model = p.get("model", "boltzmann_wealth")
        n = int(p.get("n_agents", 100))
        ns = int(p.get("n_steps", 50))
...
def _run_boltzmann(n, ns, seed):
    ...
    for i, w in enumerate(s_sorted):
        ...
        gini_num += cumul
    n * n
    gini = 1 - 2 * gini_num / (total * n) + 1 / n if total > 0 else 0
    return {
        "gini_final": float(gini),
        "mean_wealth": float(total / n),
        "n_agents": n,
        "n_steps": ns,
    }
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(
        name="abm_boltzmann_wealth_conservation",
        engine="mesa_abm",
