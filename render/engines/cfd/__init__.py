...
class SU2Adapter:
    name: str = "su2_cfd"; family: str = "cfd"; status: TrustStatus = "experimental"
    version: ClassVar[str] = "1.0.0"; runtime: ClassVar[str] = "either"
    environment: EnvSpec = EnvSpec(env_type="conda",packages=["su2>=8.0"],
        module_name="SU2",
        notes="module load SU2 on Compute2; or conda install -c conda-forge su2")
    regime: RegimeSpec = RegimeSpec(bounds=[RegimeBound(field="mach",min_val=0.01,max_val=4.9)],
        notes="Euler solver; no viscous terms.")
    @property
    def intent_schema(self): return CFDIntent
    @property
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p=inputs.params; mach=float(p.get("mach",0.3)); aoa=float(p.get("aoa_deg",2.0))
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            proc=subprocess.run([str(su2_cfd),str(cfg_file)],capture_output=True,text=True,
            ...
            cd=cl=0.0; converged=False
            if hist.exists():
                lines=hist.read_text().splitlines()
                if len(lines)>1:
                    last=lines[-1].split(",")
                    try: cd=float(last[4]); cl=float(last[5]); converged=True
                    except (IndexError,ValueError): pass
            s={"problem":p.get("problem","euler_naca0012"),"mach":mach,"aoa_deg":aoa,
               "cd":cd,"cl":cl,"converged":converged}
...
def _cfd_intent(prob,mach,aoa,title=""):
    return Intent(mode="simulation_explicit",question=title or f"CFD {prob}",family="cfd",engine="su2_cfd",
...
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(name="cfd_naca0012_cd_positive",engine="su2_cfd",
