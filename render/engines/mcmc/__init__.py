...
class EmceeAdapter:
    name: str = "emcee_mcmc"; family: str = "mcmc"; status: TrustStatus = "certified"
    ...
    @property
    def intent_schema(self): return MCMCIntent
    @property
    ...
    def run(self, inputs: EngineInputs, resources: ResourceSpec) -> RawOutputs:
        ...
        p=inputs.params; model=p.get("model","gaussian_1d")
        nw=int(p.get("n_walkers",32)); ns=int(p.get("n_steps",2000)); nb=int(p.get("n_burn",500))
        seed=int(p.get("seed",42)); rng=np.random.default_rng(seed)
        params=p.get("params",{})
        if model=="gaussian_1d":
            mu_true=float(params.get("mu_true",3.0)); sigma_true=float(params.get("sigma_true",1.0))
            n_data=int(params.get("n_data",50))
            data=rng.normal(mu_true,sigma_true,n_data)
            def log_prob(theta):
                mu,log_s=theta; s=math.exp(log_s)
                if s<=0: return -math.inf
                return -0.5*n_data*math.log(2*math.pi*s**2)-0.5*np.sum((data-mu)**2)/s**2
            ndim=2; p0=rng.normal([mu_true,math.log(sigma_true)],[0.1,0.1],(nw,ndim))
            ...
            s_vals={"mu_mean":float(flat[:,0].mean()),"mu_std":float(flat[:,0].std()),
                    "sigma_mean":float(np.exp(flat[:,1]).mean()),"mu_true":mu_true,"sigma_true":sigma_true,
...
def _mcmc_intent(model,params,n_walkers,n_steps,seed=42):
    return Intent(mode="simulation_explicit",question=f"MCMC {model}",family="mcmc",engine="emcee_mcmc",
        parameters={"model":model,"n_walkers":n_walkers,"n_steps":n_steps,"n_burn":500,"seed":seed,"params":params},
        constraints=[Constraint(name="model",value=model)])
_REFERENCE_CASES: list[ReferenceCase] = [
    ReferenceCase(name="mcmc_gaussian_1d_mean_recovery",engine="emcee_mcmc",
        ...
        intent=_mcmc_intent("gaussian_1d",{"mu_true":3.0,"sigma_true":1.0,"n_data":200},32,3000,42),
